...
test('memories reveal one at a time without leaking future text, then preserve the interview', async ({ page }) => {
  ...
  await expect(page.locator('.interview-scene')).toHaveAttribute('data-dialogue-phase', 'threshold')
  await page.screenshot({ path: 'playtest/act4-dialogue-in-cabin.png' })
  await expect(page.locator('.interview-scene')).toHaveAttribute('data-dialogue-phase', 'disembark')
  await page.waitForTimeout(520)
  await page.screenshot({ path: 'playtest/act4-dialogue-disembark.png' })
  await expect(page.locator('.interview-scene')).toHaveAttribute('data-dialogue-phase', 'manifest', { timeout: 4_000 })

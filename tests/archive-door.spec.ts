...
test('the archive waits for the profile and memory reel before opening the vault', async ({ page }) => {
  ...
  await page.waitForFunction(() => (window as unknown as { observedArchivePhases: string[] }).observedArchivePhases.includes('portal-walking'))
  ...
  await page.waitForFunction(() => (window as unknown as { observedArchivePhases: string[] }).observedArchivePhases.includes('portal-arrived'))

...
export type ActorPhase = 'idle' | 'turning' | 'walking' | 'kneeling' | 'planting' | 'rising' | 'observing' | 'touching' | 'greeting'
export type ActorEvent = { type: 'phase'; phase: ActorPhase } | { type: 'seed' | 'complete'; chapter: number; revisit: boolean }
...
export function gardenPath(from: GroundPoint, to: GroundPoint): GroundPoint[] {
  const points = [{ ...from }]
  if (from.y < 705) points.push({ x: 1380, y: 682 }, { x: 1345, y: 730 })
  points.push({ ...to })
...
export class GardenActor {
  ...
  get duration() {
    return this.phase === 'walking' ? this.travelSeconds : this.phase === 'observing' ? Math.max(.01, flowerDuration(this.chapter!) - 1.46) : ({ idle: Infinity, turning: .28, kneeling: .66, planting: .8, rising: .66, touching: .75, greeting: .7 }[this.phase])
  }
  ...
  cancel() { this.phase = 'idle'; this.elapsed = 0; this.chapter = null; this.growthElapsed = 0 }
  ...
  advance(dt: number): ActorEvent[] {
    ...
    while (remaining > 0 && this.phase !== 'idle') {
      ...
      if (previous === 'turning') this.phase = 'walking'
      else if (previous === 'walking') { this.facing = 1; this.phase = this.revisit ? 'touching' : 'kneeling' }
        ...
        this.phase = 'idle'
        if (previous !== 'greeting') completed = { type: 'complete', chapter: this.chapter!, revisit: this.revisit }
      }
      this.elapsed = 0

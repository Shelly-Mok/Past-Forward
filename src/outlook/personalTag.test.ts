...
import { ARCHIVE_PROFILE_KEY, cleanPersonalTag, emptyPersonalTag, hasPersonalTag, PERSONAL_TAG_KEY, readPersonalTag, readTagSnapshot, writePersonalTag } from './personalTag'
...
describe('personal tag', () => {
  it('keeps only the future-plan field and can recover an older thoughts save', () => {
    ...
    expect(tag).toEqual({ plans: '想换一条路', updatedAt: '2026-09-05' })
  ...
  it('reads registration and planted choices into the tag snapshot', () => {
    session.set(ARCHIVE_PROFILE_KEY, JSON.stringify({ age: '24', gender: '女', family: '与父母同住', status: '学生', rewind: '18岁' }))
    local.set(GARDEN_SAVE_KEY, JSON.stringify({
      version: 2,
      profile: { age: '24', gender: '女', family: '与父母同住', status: '学生', rewind: '18岁' },
      planted: [{ age: 18, choiceId: 'college' }, { age: 20, choiceId: 'intern' }],
    }))
    const snap = readTagSnapshot()
    expect(snap.profile.age).toBe('24')
    expect(snap.profile.status).toBe('学生')

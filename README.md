...
四幕联通的黑白像素浏览器游戏前端：海边靠近月球 → 月球回溯与穿梭 → 月面档案局五问／审查／开门 → 月面人生花园 → 三岔口进入回溯后的平行宇宙访谈与报告。

**交接基线：2026-09-14。** 完整前端流程可玩；裂隙帖子已接知乎公开搜索。第三幕已完成选择驱动的16帧种植、月面分区复色、独立基地配色和分阶段银河复色。花园时间线、选项和本地画像仍是作者骨架，接口失败时回退演示帖。Access Secret 只在本机 CLI / 后端使用，不进前端。

## 先看哪里

| 阅读目的 | 文档 |
| --- | --- |
| 换账号继续开发的当前基线和操作步骤 | [2026-09-14账号交接包](docs/ACCOUNT_HANDOFF_2026-09-14.md) |
| 可直接粘贴给新账号的接手提示词 | [新账号接手提示词](docs/NEXT_ACCOUNT_PROMPT.md) |
| 三幕完整玩法、每个操作和边界 | [队友交接总册](docs/TEAM_HANDOFF.md) |
| 第五幕报告主视觉、头像框和玻璃人物卡 | [第五幕报告布局](docs/ACT5_REPORT_LAYOUT.md) |
